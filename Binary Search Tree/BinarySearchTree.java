public class BinarySearchTree<E extends Comparable<E>> {
	...
	// Method #1.
	/**
	 * Given a value that is stored in the BST, 
	   it returns the corresponding Node that holds it. 
	...
	protected Node findNode(Node n, E val) {
		...
		else if (n.value.equals(val)) return n;
		else if (n.value.compareTo(val) > 0) {
	 ...
	 * Given a value, this method should return the “depth” of its Node, 
	   ...
	   including the root but not the node itself. 
	   The depth of the root is defined to be 0; 
	...
	protected int depth(E val) {
		...
		while (true) {
			if (n.value.equals(val)) return depth;
			else if (n.value.compareTo(val) > 0) {
				n = n.leftChild;
			} else {
				n = n.rightChild;
	...
	protected int height(E val) {
		if (val == null || findNode(val) == null) return -1;
		return height(findNode(val));
	}
	
	protected int height(Node n) {
		if (n == null) return -1;
		else if (n.leftChild == null && n.rightChild == null) return 0;
	...
	// Method #4.
	/**
	 * Given a Node, return true if the absolute value of the difference in heights of its left and right children is 0 or 1, 
	...
	// Method #5. 
	/**
	 * It returns true if isBalanced(Node) returns true for all Nodes in the tree. 
	   This method then allows the user of the BST to determine whether the BST is balanced, 
	   using the methods you’ve implemented above. 
	 * Note that the root being balanced does not imply that the entire tree is balanced.
	 */
	public boolean isBalanced() {
		return balancedFor(root);
	}

	// If balanced, the node and its left or right subtree should be balanced.
	protected boolean balancedFor(Node n) {
		if (n == null) return false;
		boolean result = isBalanced(n);
		if (n.leftChild != null) {
			result = balancedFor(n.leftChild) && result;
		}
		else if (n.rightChild != null) {
			result = balancedFor(n.rightChild) && result;
		}
		return result;
	...
	public static void main(String[] args) {
		BinarySearchTree<Integer> myBinarySearchTree = new BinarySearchTree<Integer>();
		myBinarySearchTree.add(10);
		myBinarySearchTree.add(6);
		myBinarySearchTree.add(3);
		myBinarySearchTree.add(1);
		System.err.println(myBinarySearchTree.height(3));
		System.err.println(myBinarySearchTree.isBalanced());
	}
